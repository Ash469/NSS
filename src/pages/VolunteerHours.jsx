...
// Ensure this URL is correct and matches your deployed script
...
const formatLastUpdated = (timestamp) => {
    ...
    if (typeof timestamp !== 'number' || isNaN(timestamp)) {
        return "Could not retrieve date"; // Return error message if invalid
    }
    try {
        const date = new Date(timestamp); // Create Date object from milliseconds
        // Use Intl.DateTimeFormat for locale-aware formatting
        return new Intl.DateTimeFormat('en-IN', { // 'en-IN' for India locale
            dateStyle: 'long',
            timeStyle: 'short',
        ...
        return "Error formatting date"; // Return different error message
    }
};
...
const VolunteerHours = () => {
    ...
    // Result now expects lastUpdated to be number | null
    const [result, setResult] = useState(null); // { name: '...', hours: ..., lastUpdated: 167... }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                setResult({ name: null, hours: null, lastUpdated: errorData.lastUpdated ?? null }); // Use nullish coalescing
            ...
            if (data.error) {
                setError(data.error);
                // Ensure lastUpdated is stored correctly (might be null)
            ...
            if (!result) setResult({ name: null, hours: null, lastUpdated: null }); // Ensure result is object on error
    ...
    // --- Get lastUpdated from result state for display ---
    const displayLastUpdatedTimestamp = result?.lastUpdated ?? null; // Null if not available
